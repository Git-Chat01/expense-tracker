...
const ExpenseMonthlyReport = (() => {
  ...
  function buildContext(yearMonth) {
    ...
    var prevTo = ym === today.slice(0, 7)
      ? ExpenseSummary.comparisonRange('month', from, today).to
      : _monthLastDate(prevYm);
  ...
  function _moneyStr(cents, currency) {
    return (currency || '¥') + ExpenseSummary.money(cents || 0);
  }
  ...
  function _sectionHtml(title, inner, modifier) {
    var className = 'mr-section' + (modifier ? ' mr-section--' + modifier : '');
    return '<section class="' + className + '"><h2 class="mr-section__title">'
  ...
  function _openingHtml(report) {
    var text = null;
    var cls = '';
    if (report.opening && !report.sampleGated && report.diagnoses.length) {
      return '';
    } else if (report.opening) {
    ...
    return '<aside class="mr-opening' + cls + '" role="note">'
      + '<span class="mr-opening__label">本月一句话</span>'
      + '<p class="mr-opening__text">' + _esc(text) + '</p></aside>';
  }

  /** ② 核心数据概览：总额下方独立展示比较说明，日均/笔数等宽（纯 T1，永远展示） */
  function _skeletonHtml(report) {
    var currency = report.currency || '¥';
    var changeValue;
    var changeContext;
    var changeClass = 'mr-skeleton__change';
    if (report.firstReport) {
      changeValue = '首份月报';
      changeContext = '暂无环比';
    } else if (report.changePct === null) {
      changeValue = '—';
      changeContext = '暂无可比数据';
    } else {
      var up = report.changePct >= 0;
      changeClass += ' mr-skeleton__change--' + (up ? 'up' : 'down');
      changeValue = (up ? '↑' : '↓') + Math.abs(report.changePct) + '%';
      changeContext = report.isCurrentMonth ? '比上月同期' : '比上月';
    }
    var changeHtml = '<span class="' + changeClass + '">' + changeValue + '</span>'
      + '<span class="mr-skeleton__change-context">' + changeContext + '</span>';
    return '<section class="mr-skeleton" aria-label="月度消费概览">'
      + '<div class="mr-skeleton__cell mr-skeleton__cell--total">'
      + '<p class="mr-skeleton__label">总支出</p>'
      + '<p class="mr-skeleton__value">' + _moneyStr(report.totalCents, currency) + '</p>'
      + '<div class="mr-skeleton__trend">' + changeHtml + '</div></div>'
      + '<div class="mr-skeleton__cell mr-skeleton__cell--metric">'
      + '<p class="mr-skeleton__label">日均</p>'
      + '<p class="mr-skeleton__value">' + _moneyStr(report.dailyAvgCents, currency) + '</p></div>'
      + '<div class="mr-skeleton__cell mr-skeleton__cell--metric">'
      + '<p class="mr-skeleton__label">记录</p>'
      + '<p class="mr-skeleton__value">' + report.count
      + '<span class="mr-skeleton__unit">笔</span></p></div>'
      + '</section>';
  }

  /** 诊断/洞察共用卡片：可在渲染层替换标题、略去已汇总到行动区的首条处方 */
  function _hitCardHtml(hit, cls, options) {
    options = options || {};
    var title = options.titleOverride || hit.title;
    var prescriptions = hit.prescriptions || [];
    if (options.omitFirstPrescription) prescriptions = prescriptions.slice(1);
    var html = '<article class="' + cls + '">'
      + '<h3 class="' + cls + '__title">' + _esc(title) + '</h3>'
    ...
    if (prescriptions.length) {
      html += '<div class="mr-advice"><p class="mr-advice__label">可以试试</p><ul class="' + cls + '__prescriptions">'
        + prescriptions.map(function (p) { return '<li>' + _esc(p) + '</li>'; }).join('')
        + '</ul></div>';
    }
    html += '</article>';
    return html;
  ...
  /** ③ 问题诊断区：命中规则最多 3 条；中性结论已由开场承载时不重复渲染 */
  function _diagnosesHtml(report) {
    if (report.sampleGated || !report.diagnoses.length) return '';
    return _sectionHtml('重点发现',
      report.diagnoses.map(function (hit) {
        return _hitCardHtml(hit, 'mr-diagnosis', {
          titleOverride: null,
          omitFirstPrescription: true,
        });
      }).join(''), 'findings');
  }
  ...
  function _categoriesHtml(report) {
    ...
    return _sectionHtml('钱都花在哪了', '<ul class="mr-ranking">' + rows + '</ul>' + delta, 'structure');
  }

  /** ⑤ 可选洞察区：仅 T2 命中的第 4 名起条目（覆盖率弱化说明随卡展示） */
  function _insightsHtml(report) {
    if (!report.insights.length) return '';
    return _sectionHtml('更多观察',
      report.insights.map(function (hit) { return _hitCardHtml(hit, 'mr-insight'); }).join(''), 'secondary');
  }
  ...
  function _subscriptionHtml(report) {
    ...
    var summary = report.subscription.hint
      ...
      : '<p class="mr-sub__total">合计 ' + _moneyStr(report.subscription.totalCents, report.currency)
        + '（占总支出 ' + Math.round(report.subscription.sharePct * 100) + '%）</p>';
    return _sectionHtml('订阅与固定支出',
      '<ul class="mr-sub">' + rows + '</ul>' + summary, 'secondary');
  }
  ...
  function _budgetHtml(report) {
    ...
    return _sectionHtml('预算健康度', inner, 'status');
  }
  ...
  function _actionsHtml(report) {
    ...
    return _sectionHtml('下月行动', (items ? '<ol class="mr-actions">' + items + '</ol>' : '') + nudge, 'actions');
  }

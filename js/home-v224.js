...
const ExpenseHome = (() => {
  ...
  function _renderToday(expenses) {
    ...
    const total = ExpenseSummary.sumCents(todayExpenses) / 100;
  ...
  function _renderMonth(expenses) {
    ...
    const monthExpenses = expenses.filter(e => e.date.startsWith(currentYM) && e.date <= ExpenseDB.today());
    const monthCents = ExpenseSummary.sumCents(monthExpenses);

    _$monthAmount.innerHTML = '<span class="home-overview__currency">¥</span>' + ExpenseSummary.money(monthCents);
    ...
    if (_$monthDiff) {
      ...
      if (lastMonthTotal >= MIN_BASE) {
        ...
        const diff = ExpenseSummary.changePct(monthCents, ExpenseSummary.toCents(lastMonthTotal));
  ...
  function _renderBudgetSummary() {
    ...
    var remainingCents = ExpenseSummary.toCents(monthlyBudget) - ExpenseSummary.toCents(monthSpent);
    var remainingAmount = remainingCents / 100;
    ...
    var formattedSpent = ExpenseSummary.money(ExpenseSummary.toCents(monthSpent));
    var formattedBudget = ExpenseSummary.money(ExpenseSummary.toCents(monthlyBudget));
    var formattedRemaining = ExpenseSummary.money(Math.max(0, remainingCents));
    ...
    if (_$budgetSummarySpent) _$budgetSummarySpent.innerHTML = _moneyMarkup(formattedSpent);
    if (_$budgetSummaryLimit) _$budgetSummaryLimit.innerHTML = '/ ' + _moneyMarkup(formattedBudget);
    if (_$budgetSummaryStatus) {
      _$budgetSummaryStatus.textContent = state === 'over' ? '超出预算' : (state === 'watch' ? '花得偏快' : '节奏正常');
    }

    var dailyAllowanceCents = (daysLeft > 0 && remainingCents > 0) ? Math.floor(remainingCents / daysLeft) : 0;
    ...
    if (_$budgetSummaryDecisionValue) {
      _$budgetSummaryDecisionValue.innerHTML = state === 'over'
        ? _moneyMarkup(ExpenseSummary.money(-remainingCents))
        : _moneyMarkup(ExpenseSummary.money(dailyAllowanceCents));
    }
    if (_$budgetSummaryRemaining) {
      _$budgetSummaryRemaining.innerHTML = state === 'over'
        ? '已超 ' + _moneyMarkup(ExpenseSummary.money(-remainingCents))
        : '还剩 ' + _moneyMarkup(formattedRemaining);
    }
  ...
  /* -----------------------------------------------------------------
     ...
     基础版仅含预算相关提醒（5 条规则）
     ----------------------------------------------------------------- */
  function _renderAlerts(expenses) {
    const alertsSection = document.getElementById('home-alerts-section');
    const alerts = _generateAlerts(expenses);
   ...
   * 生成提醒列表
   */
  function _generateAlerts(expenses) {
    const alerts = [];
    ...
    if (monthlyBudget > 0 && monthTotal > monthlyBudget * 0.95) {
      ...
      const remaining = (ExpenseSummary.toCents(monthlyBudget) - ExpenseSummary.toCents(monthTotal)) / 100;
      alerts.push({
        key: 'total-95', level: 'danger', icon: _ICONS.danger,
        text: remaining < 0
          ? `本月已花掉预算的 ${pct}%，已超 ¥${ExpenseSummary.money(-ExpenseSummary.toCents(remaining))}，建议控制`
          : `本月已花掉预算的 ${pct}%，仅剩 ¥${ExpenseSummary.money(ExpenseSummary.toCents(remaining))}，建议控制`,
      });
      ...
      alerts.push({
        ...
        text: `本月已花 ¥${ExpenseSummary.money(ExpenseSummary.toCents(monthTotal))}，占预算的 ${pct}%，剩余 ¥${ExpenseSummary.money(ExpenseSummary.toCents(remaining))}`,
      });
    }
    ...
    for (const [catId, catBudget] of Object.entries(catBudgets)) {
      ...
      if (spent > catBudget * 0.9) {
        ...
        alerts.push({
          ...
          text: `「${catName}」预算已使用 ${pct}%（¥${ExpenseSummary.money(ExpenseSummary.toCents(spent))}/¥${ExpenseSummary.money(ExpenseSummary.toCents(catBudget))}），注意控制`,
        });
      } else if (spent > catBudget * 0.8) {
        const pct = Math.round((spent / catBudget) * 100);
        alerts.push({
          key: `cat-${catId}-80`, level: 'warning', icon: _ICONS.warning,
          text: `「${catName}」已花 ¥${ExpenseSummary.money(ExpenseSummary.toCents(spent))}，占预算的 ${pct}%`,
        });
      }

      const prevRange = ExpenseSummary.comparisonRange('month', currentYM + '-01', ExpenseDB.today());
      const prevSpent = ExpenseSummary.sumCents(expenses.filter(e => {
        if (e.date < prevRange.from || e.date > prevRange.to) return false;
        const category = ExpenseDB.getCategory(e.categoryId);
        return e.categoryId === catId || (category && category.parentId === catId);
      })) / 100;
  ...
  function _renderRecent(expenses) {
    ...
    _$recent.innerHTML = recent.map(e => {
      ...
      if (e.date === today) {
        ...
        metaParts.push(ExpenseSummary.dateLabel(e.date, today));
      }
  ...
  function _prevYearMonth() {
    return ExpenseSummary.shiftMonths(ExpenseDB.today(), -1).slice(0, 7);
  }
  ...
  function _lastMonthTotal(expenses) {
    const today = ExpenseDB.today();
    const from = _prevYearMonth() + '-01';
    const to = ExpenseSummary.shiftMonths(today, -1);
    return ExpenseSummary.sumCents(expenses.filter(e => e.date >= from && e.date <= to)) / 100;
  }

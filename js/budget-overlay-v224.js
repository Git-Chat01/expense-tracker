...
const ExpenseBudgetOverlay = (() => {
  ...
  function open() {
    ...
    const remainingCents = ExpenseSummary.toCents(monthlyBudget) - ExpenseSummary.toCents(monthTotal);
    const budgetBalance = `${remainingCents < 0 ? '已超' : '剩余'} ¥${ExpenseSummary.money(Math.abs(remainingCents))}`;
    ...
    body.innerHTML = `
      <div style="margin-bottom:24px">
        ...
        ${monthlyBudget > 0 ? `<p style="margin-top:8px;font-size:13px;color:var(--color-text-secondary);text-align:center">已用 ¥${ExpenseSummary.money(ExpenseSummary.toCents(monthTotal))} · ${budgetBalance}</p>` : ''}
      </div>
          ...
          ${ExpenseDB.getParentCategories().map(cat => {
            ...
            return `
              <div style="display:flex;align-items:center;gap:8px">
                ...
                ${catBudget > 0 ? `<span style="font-size:11px;color:var(--color-text-tertiary);width:60px;text-align:right">${ExpenseSummary.toCents(spent) > ExpenseSummary.toCents(catBudget) ? '<svg viewBox="0 0 24 24" class="inline-icon" aria-hidden="true"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m21.73 18l-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3M12 9v4m0 4h.01"/></svg>超支' : Math.round(spent/catBudget*100)+'%'}</span>` : '<span style="width:60px"></span>'}
              </div>`;
          }).join('')}
        </div>
      </div>

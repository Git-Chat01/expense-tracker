...
const ExpenseMonthlyReport = (() => {
  ...
  /** ② 核心数据概览：总额与环比同处主视觉，日均/笔数等宽（纯 T1，永远展示） */
  function _skeletonHtml(report) {
    ...
    return '<section class="mr-skeleton" aria-label="月度消费概览">'
      ...
      + '<div class="mr-skeleton__topline"><p class="mr-skeleton__label">总支出</p>'
      + '<div class="mr-skeleton__trend"><span class="mr-skeleton__trend-label">环比</span>'
      + changeHtml + '</div></div>'
  ...
  function _diagnosesHtml(report) {
    ...
    return _sectionHtml('重点发现',
      report.diagnoses.map(function (hit, i) {
        return _hitCardHtml(hit, 'mr-diagnosis', {
          titleOverride: i === 0 ? '为什么这么说' : null,

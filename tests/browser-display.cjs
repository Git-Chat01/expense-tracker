  ...
  try {
    ...
    const fixture = await page.evaluate(() => {
      ...
      for (let i = 0; i < dates.length; i++) {
        if (!ExpenseDB.addExpense({ amount: amounts[i], date: dates[i], categoryId: cats[i % cats.length], time: '12:30', paymentMethod: 'wechat', necessity: i % 2 ? 'need' : 'impulse', note: '回归测试' })) throw Error('测试记录写入失败');
      }
